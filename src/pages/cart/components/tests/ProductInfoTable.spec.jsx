...
it('특정 아이템의 수량이 변경되었을 때 값이 재계산되어 올바르게 업데이트 된다', async () => {
  ...
  expect(screen.getByText(formatPrice(2427 + 809 * 2))).toBeInTheDocument();
});
...
it('특정 아이템의 삭제 버튼을 클릭할 경우 해당 아이템이 사라진다', async () => {
  ...
  // getBy~~: 요소가 존재하지 않으면 에러 발생
  // 요소가 DOM에 존재하지 않음을 단언할 때는 queryBy~~ 사용하는 것이 좋고, 평소에는 정확한 에러 피드백을 주는 getBy~~ 사용
